...
public class InsightsServiceImpl implements InsightsService {
    ...
    public UserAdvancedAnalyticsDTO getAdvancedInsights(Long userId, Long requesterId) {
        ...
        return UserAdvancedAnalyticsDTO.builder()
                .accountBalance(accountBalance)
                .outstandingLoans(outstandingLoans)
                .currentNetWorth(accountBalance.subtract(outstandingLoans))
                .rewardBalance(insightsRepository.rewardBalance(userId))
                .spendingBreakdown(insightsRepository.findSpendingBreakdown(userId, currentMonth.atDay(1).atStartOfDay())
                        .stream()
                        .map(this::mapPoint)
                        .toList())
                .budgetVsActual(insightsRepository.findBudgetVsActual(userId, currentMonth.toString())
                        .stream()
                        .map(this::mapPoint)
                        .toList())
                .netWorthProgression(buildNetWorthProgression(userId, sixMonthsAgo, accountBalance, outstandingLoans))
                .rewardGrowth(insightsRepository.findRewardGrowth(userId, sixMonthsAgo)
                        .stream()
                        .map(this::mapPoint)
                        .toList())
                .loanPayoffForecast(insightsRepository.findLoanPayoffForecast(userId)
                        .stream()
                        .limit(12)
                        .map(this::mapPoint)
                        .toList())
    ...
    public SpendingAnalyticsDTO getSpendingAnalytics(Long userId, Long requesterId) {
        ...
        return SpendingAnalyticsDTO.builder()
                .categorySpending(insightsRepository.findSpendingBreakdown(userId,
    ...
    public WealthAnalyticsDTO getWealthAnalytics(Long userId, Long requesterId) {
        ...
        return WealthAnalyticsDTO.builder()
                .accountBalance(accountBalance)
                .outstandingLoanPrincipal(outstandingLoans)
                .netWorth(accountBalance.subtract(outstandingLoans))
                .netWorthProgression(buildNetWorthProgression(userId, sixMonthsAgo, accountBalance, outstandingLoans))
                .rewardGrowth(insightsRepository.findRewardGrowth(userId, sixMonthsAgo).stream()
                        .map(this::mapPoint)
                        .toList())
                .build();
    }
    ...
    public LoanPayoffForecastDTO getLoanPayoffForecast(Long userId, Long requesterId) {
        ...
        return LoanPayoffForecastDTO.builder()
    ...
    private List<AnalyticsPointDTO> buildNetWorthProgression(Long userId, LocalDateTime startDate,
        ...
        insightsRepository.findMonthlyNetMovement(userId, startDate)
                .forEach(row -> movementByMonth.computeIfPresent(row.getLabel(),
                        (key, value) -> value.add(safe(row.getValue()))));

        BigDecimal running = currentNetWorth.subtract(movementByMonth.values().stream()

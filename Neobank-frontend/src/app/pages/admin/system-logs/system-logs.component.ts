  ...
  template: `
    <section class="logs-shell">
      <div class="title-row">
        ...
        <button class="primary-btn" (click)="exportCsv()" [disabled]="logs.length === 0">Export CSV</button>
      </div>
      ...
      <div class="chart-grid">
        <div class="panel"><h3>Error Rate</h3><canvas #errorChart></canvas></div>
        <div class="panel"><h3>Response Time</h3><canvas #responseChart></canvas></div>
      </div>
      ...
      <div class="panel table-panel">
          ...
          <table>
            ...
            <tbody>
              <tr *ngFor="let log of logs">
                ...
                <td><span class="status" [class.error]="log.responseStatus >= 400">{{ log.responseStatus }}</span></td>
        ...
        <div class="pager">
          ...
          <button (click)="load(page + 1)" [disabled]="page + 1 >= totalPages || loading">Next</button>
        </div>
      </div>
    </section>
  `,
  styles: [`
    .logs-shell { padding: clamp(1rem, 2vw, 2rem); color: #0f172a; min-width: 0; }
    :host-context(.dark-mode) .logs-shell { color: #f8fafc; }
    .title-row { display: flex; justify-content: space-between; align-items: center; gap: 1rem; margin-bottom: 1rem; }
    h2, h3, p { margin: 0; } p { color: #64748b; margin-top: .35rem; }
    :host-context(.dark-mode) p { color: #a8bddf; }
    .filters { display: grid; grid-template-columns: repeat(6, minmax(0, 1fr)); gap: .75rem; margin-bottom: 1rem; }
    input { width: 100%; min-height: 44px; border: 1px solid #cbd5e1; border-radius: 8px; padding: .65rem .8rem; background: #fff; color: #0f172a; }
    :host-context(.dark-mode) input { background: #0f172a; color: #e5e7eb; border-color: #334155; }
    .primary-btn, .pager button { min-height: 44px; border: 0; border-radius: 8px; padding: .65rem 1rem; background: #2563eb; color: white; font-weight: 700; cursor: pointer; }
    button:disabled { opacity: .55; cursor: not-allowed; }
    .chart-grid { display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); gap: 1rem; margin-bottom: 1rem; }
    .panel { background: #fff; border: 1px solid #e2e8f0; border-radius: 12px; padding: 1rem; box-shadow: 0 12px 30px rgba(15,23,42,.08); min-width: 0; }
    :host-context(.dark-mode) .panel { background: rgba(22,33,62,.94); border-color: rgba(96,165,250,.16); }
    canvas { width: 100% !important; height: 240px !important; margin-top: 1rem; }
    .table-scroll { overflow-x: auto; -webkit-overflow-scrolling: touch; }
    table { width: 100%; min-width: 920px; border-collapse: collapse; }
    th, td { padding: .75rem; border-bottom: 1px solid #e2e8f0; text-align: left; vertical-align: top; }
    :host-context(.dark-mode) th, :host-context(.dark-mode) td { border-color: rgba(148,163,184,.18); }
    th { color: #475569; font-size: .82rem; text-transform: uppercase; }
    :host-context(.dark-mode) th { color: #cbd5e1; }
    .endpoint { max-width: 280px; overflow-wrap: anywhere; }
    .status { padding: .25rem .55rem; border-radius: 999px; background: #dcfce7; color: #047857; font-weight: 800; }
    ...
    @media (max-width: 768px) {
      .title-row, .chart-grid, .filters { grid-template-columns: 1fr; display: grid; align-items: stretch; }
      .primary-btn { width: 100%; }
      .pager { flex-direction: column; align-items: stretch; }
      canvas { height: 210px !important; }
    }
  `],
})
export class SystemLogsComponent implements OnInit, OnDestroy {
  ...
  constructor(private service: AdminDashboardService, private themeService: ThemeService) {
    effect(() => {
  ...
  private renderCharts(): void {
    ...
    this.addChart(this.errorChartRef, {
      ...
      data: {
        ...
        datasets: [{ data: [this.logs.filter(l => l.responseStatus < 400).length, this.logs.filter(l => l.responseStatus >= 400).length], backgroundColor: ['#10b981', '#ef4444'] }],
      },
    ...
    this.addChart(this.responseChartRef, {
      ...
      data: {
        ...
        datasets: [{ label: 'Response ms', data: this.logs.slice(0, 10).map((log) => log.executionTimeMs), backgroundColor: '#3b82f6' }],
      },
  ...
  private chartOptions(type?: string): any {
    ...
    return {
      ...
      ...(type !== 'doughnut' && { scales: { x: { ticks: { color }, grid: { color: grid } }, y: { ticks: { color }, grid: { color: grid }, beginAtZero: true } } }),
    };
  }

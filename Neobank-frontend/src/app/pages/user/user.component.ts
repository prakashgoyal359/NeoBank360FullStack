...
export class UserComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  refreshDashboardCharts(): void {
    ...
    if (this.selectedAccount) {
      this.bankingService.getAccountTransactions(this.selectedAccount.id).subscribe({
        next: (transactions) => {
          this.transactions = transactions;
        ...
        error: (error) => console.error('Error refreshing transactions:', error),
      });
    } else {
      this.processChartData();
    }
  ...
  setActiveSection(section: string): void {
    ...
    // ✅ RECREATE CHART when coming back to home
